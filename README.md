...
Before rendering, the graphics builder creates a structured portrait layout for each scene, then a separate coding stage translates that approved layout into isolated scene-specific HTML, scoped CSS, inline SVG, and deterministic JavaScript. Independent visual beats run on three bounded scene runners by default; set `SVF_CUSTOM_GRAPHICS_CONCURRENCY=1` through `4` to override that limit. The accepted source, individual inspectable scene previews, visual-QA evidence, and a master graphics preview live under `08_graphics/`. Every semantic action is narration-anchored, every future element remains hidden until its cue, and measured failures repair only the affected scene. Mock/offline generation retains the legacy deterministic object renderer. The composition compiler then creates a complete, voice-timed interactive preview under `09_composition/preview/`; run `svf prepare-preview <episode-id>` to refresh it without rendering an MP4. Every scene is a `.clip` with `data-start` and `data-duration`. The page listens to `hf-seek` and recomputes scene state from the render clock, so browser preview and frame rendering use the same timeline semantics.
