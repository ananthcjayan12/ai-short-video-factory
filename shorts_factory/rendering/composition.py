...
initializeCustomGraphics();function renderAt(t){currentTime=clamp(Number(t)||0,0,DURATION);let activeCaption='';for(const item of ENTRIES){const el=document.getElementById(item.id);if(!el)continue;const active=currentTime>=item.start&&currentTime<item.end;if(!active){el.style.opacity='0';continue}const d=Math.max(.001,item.end-item.start),p=(currentTime-item.start)/d;const intro=ease(clamp(p/.055)),outro=ease(clamp((1-p)/.055));el.style.opacity=String(outro);el.style.clipPath=`inset(0 ${Math.max(0,(1-intro)*8)}% 0 0)`;activeCaption=item.caption||'';const graphic=el.classList.contains('graphic')||el.classList.contains('diagram');if(graphic&&!el.classList.contains('generated-graphic')&&!el.classList.contains('custom-generated-graphic'))el.style.transform=`scale(${.985+.015*ease(p)})`;if(el.classList.contains('generated-graphic'))renderGeneratedGraphic(el,Math.max(0,currentTime-item.start),d);if(el.classList.contains('custom-generated-graphic'))renderCustomGraphicScene(el,Math.max(0,currentTime-item.start),d);const vid=el.matches('video')?el:el.querySelector('video');if(vid){const local=Math.max(0,currentTime-item.start);if(Number.isFinite(vid.duration)&&vid.duration>.05){const wanted=Math.min(local,Math.max(0,vid.duration-.04));if(Math.abs((vid.currentTime||0)-wanted)>.08)vid.currentTime=wanted;}vid.pause();}}caption.textContent=activeCaption;seek.value=String(currentTime);timeLabel.textContent=`${currentTime.toFixed(1)} / ${DURATION.toFixed(1)}s`;}
