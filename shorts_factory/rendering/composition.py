...
from ..custom_graphics import (
    CUSTOM_GRAPHICS_RUNTIME, CustomGraphicsPackage, CustomGraphicsSceneBundle,
    custom_factory_registration, custom_scene_markup,
)
from ..custom_graphics.compiler import CUSTOM_GRAPHICS_STYLES
...
def _visual_markup(
    ...
    custom_scene: CustomGraphicsSceneBundle | None = None,
) -> str:
    if custom_scene:
        return custom_scene_markup(
            custom_scene, element_id, start=float(scene["start"]), duration=float(scene["duration"]),
        )
    if graphics_scene:
...
def _scene_markup(
    ...
    return _visual_markup(scene, element_id, graphics_scene, graphics_theme, custom_scene)
...
def build(project_dir: Path, *, preview: bool, width: int, height: int, fps: int = 60,
    ...
    custom_path = project_dir / "08_graphics/custom_graphics.json"
    custom = load_model(custom_path, CustomGraphicsPackage) if custom_path.is_file() else None
    custom_by_scene = {
        scene.layout.scene_id: scene for scene in custom.scenes
    } if custom else {}
    ...
    manifest = {
        ...
        "custom_graphics_scenes": {
            scene_id: value.model_dump(mode="json") for scene_id, value in custom_by_scene.items()
        },
    }
...
def _html(manifest: dict[str, Any]) -> str:
    ...
    custom_by_scene = {
        scene_id: CustomGraphicsSceneBundle.model_validate(value)
        for scene_id, value in manifest.get("custom_graphics_scenes", {}).items()
    }
    entries_markup = "\n".join(
        _scene_markup(
            item, i, graphics_by_scene.get(item["scene_id"]),
            manifest.get("graphics_theme", "editorial"),
            custom_by_scene.get(item["scene_id"]),
        )
    ...
    entries_json = json.dumps([
        {
            ...
            "type": e["type"], "purpose": e.get("purpose", ""),
        }
    ...
    custom_registrations = "\n".join(
        custom_factory_registration(bundle) for bundle in custom_by_scene.values()
    )
...
__ENTRIES__<div id="grain"></div><div id="brand">AI WORKFLOW CASE STUDY</div></div>
__AUDIO__
...
__CUSTOM_GRAPHICS_RUNTIME__
__CUSTOM_GRAPHICS_REGISTRATIONS__
const ENTRIES=__ENTRIES_JSON__,DURATION=__DURATION__;const stage=document.getElementById('stage'),audio=document.getElementById('master-audio'),seek=document.getElementById('timeline-seek'),timeLabel=document.getElementById('timeline-time'),toggle=document.getElementById('timeline-toggle'),rail=document.getElementById('scene-rail');const clamp=(v,a=0,b=1)=>Math.max(a,Math.min(b,v));const ease=v=>1-Math.pow(1-clamp(v),3);let currentTime=0,playing=false;
...
initializeCustomGraphics();function renderAt(t){currentTime=clamp(Number(t)||0,0,DURATION);for(const item of ENTRIES){const el=document.getElementById(item.id);if(!el)continue;const active=currentTime>=item.start&&currentTime<item.end;if(!active){el.style.opacity='0';continue}const d=Math.max(.001,item.end-item.start),p=(currentTime-item.start)/d;const intro=ease(clamp(p/.055)),outro=ease(clamp((1-p)/.055));el.style.opacity=String(outro);el.style.clipPath=`inset(0 ${Math.max(0,(1-intro)*8)}% 0 0)`;const graphic=el.classList.contains('graphic')||el.classList.contains('diagram');if(graphic&&!el.classList.contains('generated-graphic')&&!el.classList.contains('custom-generated-graphic'))el.style.transform=`scale(${.985+.015*ease(p)})`;if(el.classList.contains('generated-graphic'))renderGeneratedGraphic(el,Math.max(0,currentTime-item.start),d);if(el.classList.contains('custom-generated-graphic'))renderCustomGraphicScene(el,Math.max(0,currentTime-item.start),d);const vid=el.matches('video')?el:el.querySelector('video');if(vid){const local=Math.max(0,currentTime-item.start);if(Number.isFinite(vid.duration)&&vid.duration>.05){const wanted=Math.min(local,Math.max(0,vid.duration-.04));if(Math.abs((vid.currentTime||0)-wanted)>.08)vid.currentTime=wanted;}vid.pause();}}seek.value=String(currentTime);timeLabel.textContent=`${currentTime.toFixed(1)} / ${DURATION.toFixed(1)}s`;}
...
for(const item of ENTRIES){const button=document.createElement('button');button.type='button';button.className='scene-jump';button.style.flexGrow=String(Math.max(.1,item.end-item.start));button.dataset.type=item.type;button.dataset.label=`${item.scene_id} · ${item.purpose}`;button.title=button.dataset.label;button.addEventListener('click',()=>{setPlaying(false);renderAt(Math.min(item.end-.01,item.start+.35));if(audio)audio.currentTime=currentTime;});rail.appendChild(button);}
  ...
  window.__svfRenderAt=renderAt;window.addEventListener('hf-seek',event=>{setPlaying(false);renderAt(Number(event.detail.time||0));});renderAt(0);window.__hf_ready__=true;
    ...
    for key, value in {
        ...
        "__CUSTOM_GRAPHICS_STYLES__": CUSTOM_GRAPHICS_STYLES,
        "__CUSTOM_GRAPHICS_RUNTIME__": CUSTOM_GRAPHICS_RUNTIME,
        "__CUSTOM_GRAPHICS_REGISTRATIONS__": custom_registrations,
    }.items():
